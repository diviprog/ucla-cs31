...
{
    if (r == player()->row() && c == player()->col()) return true;
    else return false;
}
...
int City::nTootersAt(int r, int c) const
{
    int num_tooters = 0;
    for (int i = 0; i < m_nTooters; i++) {
        if ((r == m_tooters[i]->row()) && (c == m_tooters[i]->col())) {
            num_tooters++;
        }
    }
    return num_tooters;
...
void City::display() const
{
    ...
    for (int i = 0; i < m_nTooters; i++) {
        ...
        else if (isdigit(grid[row][col]) and grid[row][col]!='9') {
            grid[row][col]++;
        }
    }
...
void City::moveTooters()
{
    ...
    {
        m_tooters[k]->move();
        int t_r = m_tooters[k]->row();
        int t_c = m_tooters[k]->col();
        int p_r = m_player->row();
        int p_c = m_player->col();
        if ((p_c == t_c+1 && p_r == t_r) || (p_c == t_c-1 && p_r == t_r) || (p_c == t_c && p_r == t_r-1) || (p_c == t_c && p_r == t_r+1)){
            m_player->getGassed();
        }
    }
}
