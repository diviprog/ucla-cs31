...
void Tooter::move()
{
    ...
    if (dir == UP && m_row!=1 && not(m_city->isPlayerAt(m_row-1, m_col))){
        m_row--;
    }
    else if (dir == DOWN && m_row!=(m_city->rows()) && not(m_city->isPlayerAt(m_row+1, m_col))){
        m_row++;
    }
    else if (dir == LEFT && m_col!=1 && not(m_city->isPlayerAt(m_row, m_col-1))) {
        m_col--;
    }
    else if (dir == RIGHT && m_col!=(m_city->cols()) && not(m_city->isPlayerAt(m_row, m_col+1))){
        m_col++;
    }
}
...
{
    m_age++;
    if (dir == UP && m_row!=1 && (m_city->nTootersAt(m_row-1, m_col) == 0)){
        m_row--;
    }
    else if (dir == DOWN && m_row!=(m_city->rows()) && (m_city->nTootersAt(m_row+1, m_col) == 0)){
        m_row++;
    }
    else if (dir == LEFT && m_col!=1 && (m_city->nTootersAt(m_row, m_col-1) == 0)) {
        m_col--;
    }
    else if (dir == RIGHT && m_col!=(m_city->cols()) && (m_city->nTootersAt(m_row, m_col+1) == 0)){
        m_col++;
    }
}
...
int City::nTootersAt(int r, int c) const
{
    int num_tooters = 0;
    for (int i = 0; i < m_nTooters; i++) {
        if (m_tooters[i]->row()==r && m_tooters[i]->col()==c) num_tooters++;
    }
    return num_tooters;
}
...
void City::display() const
{
    ...
    for (int i = 0; i < m_nTooters; i++) {
        int number_tooters = nTootersAt(m_tooters[i]->row(), m_tooters[i]->col());
        if (number_tooters==1) {
            grid[m_tooters[i]->row()-1][m_tooters[i]->col()-1] = 'T';
        }
        else if (number_tooters>=9) {
            grid[m_tooters[i]->row()-1][m_tooters[i]->col()-1] = '9';
        }
        else {
            grid[m_tooters[i]->row()-1][m_tooters[i]->col()-1] = '0'+number_tooters;
        }
    }
...
void City::preachToTootersAroundPlayer()
{
    ...
    for (int i = 0; i < m_nTooters; i++) {
        int t_row = m_tooters[i]->row();
        int t_col = m_tooters[i]->col();
        if ((t_row==p_row && t_col==p_col-1) or
            (t_row==p_row && t_col==p_col+1) or
            (t_row==p_row-1 && t_col==p_col) or
            (t_row==p_row+1 && t_col==p_col) or
            (t_row==p_row+1 && t_col==p_col+1) or
            (t_row==p_row+1 && t_col==p_col-1) or
            (t_row==p_row-1 && t_col==p_col-1) or
            (t_row==p_row-1 && t_col==p_col+1)) {
            if (randInt(0, 2) != 0) {
                delete m_tooters[i];
...
void City::moveTooters()
{
    ...
    {
        m_tooters[k]->move();
        int t_row = m_tooters[k]->row();
        int t_col = m_tooters[k]->col();
        int p_row = m_player->row();
        int p_col = m_player->col();
        if ((t_row==p_row && t_col==p_col-1) or (t_row==p_row && t_col==p_col+1) or (t_row==p_row-1 && t_col==p_col) or (t_row==p_row+1 && t_col==p_col)) m_player->getGassed();
    }
}
